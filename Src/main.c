 ...
 while(1)
 {
  ...
  // HAL_SPI_DMAPause(&hspi2);
   ...
   if (f_read(&file,TrackBuffer+track_offset,sizeof(uint8_t)*BLOCK_SIZE,&readen)!=FR_OK)
    ...
    f_close(&file);		 
    cDisplayStandardLibrary.PutString(0,CDisplayStandardLibrary::FONT_HEIGHT*5,"Сбой 1",IDisplay::COLOR_BLACK);
		HAL_Delay(5000);
		return(false);
	 }
    ...
    HAL_SPI_DMAStop(&hspi2);		 
    f_close(&file);
    cDisplayStandardLibrary.PutString(0,CDisplayStandardLibrary::FONT_HEIGHT*5,"Сбой 2",IDisplay::COLOR_BLACK);
    HAL_Delay(5000);
		return(false);
	 }	
	 size_t counter=0;
